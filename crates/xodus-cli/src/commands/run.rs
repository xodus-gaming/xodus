...
pub async fn run(
    ...
    match xodus::playtime::record_session(&content_id, elapsed) {
        Ok(total_seconds) => println!(
            "Session ended. Played for {} (Total: {})",
            xodus::playtime::PlayTimeStore::format_duration(elapsed.as_secs()),
            xodus::playtime::PlayTimeStore::format_duration(total_seconds)
        ),
        Err(error) => {
            eprintln!("Warning: Failed to save playtime: {error}");
        }
    }
