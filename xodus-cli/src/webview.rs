    ...
    let builder = WebViewBuilder::new()
            ...
            .with_ipc_handler(move |request| {
                ...
                if let Ok(data) = payload {
                    if proxy_ipc.send_event(CustomEvent::IpcCallback(data)).is_err() {
                    ...
                    match serde_json::from_str::<HostBridgeMessage>(body) {
                        Ok(message) => {
                            if let Some(ctx) = message.get_context_invoke() {
                                if proxy_ipc
                                    .send_event(CustomEvent::HostGetContext(ctx.to_string()))
                                    .is_err()
                                {
        ...
        *control_flow = ControlFlow::Wait;
        match event {
            ...
            Event::UserEvent(CustomEvent::HostGetContext(ctx)) => {
                _ = webview.evaluate_script(&format!(r#"window["CloudExperienceHost.Bridge.dispatchMessage"](JSON.stringify({{"type": "callback", "value": {{ "name": "CloudExperienceHost.getContext", "args": ["CloudExperienceHost", "TokenBroker", "TokenBroker", "{{\"PrivatePropertyBag\":1,\"PasswordlessConnect\":1,\"PreferAssociate\":1,\"ChromelessUI\":0}}"], "context": "{ctx}"}}}}))"#));
            }
...
pub fn login_client(
    ...
    run_webview(url, headermap)
}

pub async fn finalize(url: String) -> Result<Option<DAProperty>, Box<dyn std::error::Error>> {
    run_webview(url, HeaderMap::new())
}
